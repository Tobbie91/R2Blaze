    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="border rounded-xl p-4">
        ...
        <div className="grid gap-2">
          ...
          <button type="button" onClick={onSave} className="px-4 py-2 rounded bg-emerald-700 text-white">
            {editingId ? 'Save changes' : 'Save product'}
          </button>
          {!editingId && (
            <p className="text-xs text-gray-600">Products save to this device (localStorage). For multi-device sync later we can add Firebase.</p>
      ...
      <div className="border rounded-xl p-4">
        ...
        <ul className="space-y-2 text-sm">
          {products.map(p => (
            <li key={p.id} className={`flex items-center justify-between border rounded p-2 ${editingId === p.id ? 'ring-1 ring-emerald-500' : ''}`}>
              ...
              <div className="flex items-center gap-2">
                <Link to={`/products/${p.slug}`} className="px-3 py-1 rounded border hover:bg-gray-50">View</Link>
                <button type="button" onClick={() => startEdit(p)} className="px-3 py-1 rounded border hover:bg-gray-50">Edit</button>
                <button type="button" onClick={() => onDelete(p.id, p.name)} className="px-3 py-1 rounded border border-red-600 text-red-600 hover:bg-red-50">Delete</button>
              </div>
            </li>
          ))}
          {!products.length && <li className="text-gray-500">No products yet.</li>}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() => {
              const data = localStorage.getItem('r2blaze_catalog') || '[]'
              const blob = new Blob([data], { type: 'application/json' })
              const a = document.createElement('a')
              a.href = URL.createObjectURL(blob)
              a.download = 'r2blaze_catalog.json'
              a.click()
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  localStorage.setItem('r2blaze_catalog', JSON.stringify(arr))
                  alert('Catalog imported! Reloading…')
                  location.reload()
                } catch (err: any) {
        ...
        {/* Seed button (optional, for quick testing) */}
        <button
          type="button"
          onClick={() => {
            try {
              ...
              localStorage.setItem('r2blaze_catalog', JSON.stringify(seeded))
